  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex flex-col md:flex-row flex-1">
        {/* Hospital List */}
        <div 
          className="w-full md:w-1/2 overflow-y-auto order-2 md:order-1" 
          ...
          <div className="p-3 md:p-4 space-y-3 md:space-y-4">
                ...
                <div className="p-3 md:p-4">
                  <div className="flex gap-3 md:gap-4">
                    <img
                      src={hospital.image}
                      alt={hospital.name}
                      className="w-16 h-16 md:w-28 md:h-28 rounded-lg object-cover"
                    />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between mb-1 md:mb-2">
                        ...
                        <span className="flex items-center gap-1 px-2 py-1 bg-green-50 text-green-700 rounded-lg text-xs md:text-sm">
                          <Star className="w-3 h-3 md:w-4 md:h-4 fill-current" />
                          {hospital.rating}
                        </span>
                      </div>
                      <div className="flex items-center gap-1 text-gray-500 mb-1 md:mb-2 text-xs md:text-sm">
                        <MapPin className="w-3 h-3 md:w-4 md:h-4 flex-shrink-0" />
                        <span className="truncate">{hospital.address}</span>
                      </div>
                      <div className="flex items-center gap-2 mb-2">
                        <span className="inline-block px-2 py-0.5 text-xs md:text-sm text-blue-600 bg-blue-50 rounded-lg">
                          {hospital.distance}
                        </span>
                      </div>
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <div className="flex flex-wrap gap-1 md:gap-2">
                          {hospital.services.map((service, index) => (
                            <span
                              key={index}
                              className="px-2 py-0.5 text-xs md:text-sm bg-gray-100 text-gray-600 rounded-lg"
                            >
                              {service}
                            </span>
                          ))}
                        ...
                        <button
                          className="px-3 md:px-6 py-1.5 text-xs md:text-base bg-[#98E9E9] hover:bg-[#7CD5D5] text-gray-700 rounded-lg transition-colors"
            ...
            {loading && (
              <div className="text-center py-4">
                <div className="w-6 h-6 md:w-8 md:h-8 border-4 border-[#98E9E9] border-t-transparent rounded-full animate-spin mx-auto"></div>
              </div>
            )}
          </div>
        </div>
        ...
        {/* Map */}
        <div 
          className="w-full md:w-1/2 order-1 md:order-2 sticky top-0" 
          style={{ height: ["100vh - 200px", "300px"] }}
        >
          <iframe
