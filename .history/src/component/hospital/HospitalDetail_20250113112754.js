...
const HospitalDetail = () => {
  ...
  const [galleryWithComments, setGalleryWithComments] = useState([]);
  const [userLikes, setUserLikes] = useState(new Set());

  // Khởi tạo galleryWithComments sau khi có dữ liệu hospital
  useEffect(() => {
    if (hospital?.gallery) {
      setGalleryWithComments(
              ...
              <div key={image.id} className="bg-gray-50 rounded-xl overflow-hidden">
                <div className="aspect-video relative group">
                  ...
                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <button className="text-white">Click to view details</button>
                  </div>
                </div>
                <div className="p-4">
