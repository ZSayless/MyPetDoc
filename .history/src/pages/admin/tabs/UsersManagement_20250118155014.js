...
function UsersManagement() {
  ...
  const handleAddUser = () => {
    ...
    if (!newUser.name.trim()) {
      setErrors((prev) => ({ 
        ...prev, 
        name: t("admin.users.modal.validation.nameRequired") 
      }));
      hasError = true;
    }
    ...
    if (!newUser.email.trim()) {
      setErrors((prev) => ({ 
        ...prev, 
        email: t("admin.users.modal.validation.emailRequired") 
      }));
      hasError = true;
    } else if (!isValidEmail(newUser.email)) {
      setErrors((prev) => ({
        ...prev,
        email: t("admin.users.modal.validation.emailInvalid"),
      }));
      hasError = true;
    }
    ...
    if (!newUser.password) {
      setErrors((prev) => ({ 
        ...prev, 
        password: t("admin.users.modal.validation.passwordRequired") 
      }));
      hasError = true;
    } else if (newUser.password.length < 6) {
      setErrors((prev) => ({
        ...prev,
        password: t("admin.users.modal.validation.passwordLength"),
      }));
      hasError = true;
    }
    ...
    if (!newUser.confirmPassword) {
      setErrors((prev) => ({
        ...prev,
        confirmPassword: t("admin.users.modal.validation.confirmRequired"),
      }));
      hasError = true;
    } else if (newUser.password !== newUser.confirmPassword) {
      setErrors((prev) => ({
        ...prev,
        confirmPassword: t("admin.users.modal.validation.passwordsNotMatch"),
      }));
      hasError = true;
    }
            ...
            <div className="p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  >
                    <option value="Pet Owner">{t("admin.users.modal.roles.petOwner")}</option>
                    <option value="Veterinarian">{t("admin.users.modal.roles.veterinarian")}</option>
                ...
                <div>
                  ...
                  {modalMode === "view" ? (
                    ...
                    <select
                      ...
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                    >
                      <option value="active">{t("admin.users.modal.statuses.active")}</option>
                      <option value="inactive">{t("admin.users.modal.statuses.inactive")}</option>
                    </select>
                  )}
                </div>
              </div>
                ...
                {modalMode === "view" ? (
                  <>
                    <button
                      ...
                      className="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200"
                    >
                      {t("admin.users.modal.buttons.close")}
                    </button>
                    <button
                      onClick={() => setModalMode("edit")}
                      className="px-4 py-2 bg-[#98E9E9] text-gray-700 rounded-lg hover:bg-[#7CD5D5]"
                    >
                      {t("admin.users.modal.buttons.edit")}
                    </button>
                  </>
                ) : (
                  <>
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-[#98E9E9] text-gray-700 rounded-lg hover:bg-[#7CD5D5]"
                    >
                      {t("admin.users.modal.buttons.save")}
                    </button>
                  </>
                )}
              </div>
