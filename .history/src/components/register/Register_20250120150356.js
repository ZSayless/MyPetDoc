...
import { authService, registerWithGoogle } from "../../services/authService";
...
function Register({ isOpenRegister, onLoginClick, onClose }) {
  ...
  const roles = [
    {
      value: "user",
      label: t("auth.selectRole.roles.petOwner.title"),
    },
    {
      value: "veterinarian",
      label: t("auth.selectRole.roles.veterinarian.title"),
    },
  ];
  ...
  const validateForm = () => {
    ...
    if (!formData.role) {
      setError(t("auth.errors.roleRequired"));
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(
        error.response?.data?.message || t("auth.errors.registrationFailed")
      );
