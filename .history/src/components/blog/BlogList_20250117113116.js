...
function BlogList() {
  ...
  const [selectedCategories, setSelectedCategories] = useState([t("blog.list.categories.allPosts")]);
  ...
  const handleCategoryToggle = (category) => {
    if (category === t("blog.list.categories.allPosts")) {
      ...
      setSelectedCategories((prev) => {
        const newSelection = prev.filter(cat => cat !== t("blog.list.categories.allPosts"));
        
        if (prev.includes(category)) {
          const filtered = newSelection.filter(cat => cat !== category);
          return filtered.length === 0 ? [t("blog.list.categories.allPosts")] : filtered;
        } else {
                ...
                <div className="p-6">
                  ...
                  <div className="flex items-center justify-between pt-4 border-t">
                    ...
                    <div className="flex items-center gap-4 text-sm text-gray-500">
                      <span>{post.likes} {t("blog.list.card.likes")}</span>
                      <span>{post.comments} {t("blog.list.card.comments")}</span>
                    </div>
                  </div>
                </div>
              </Link>
