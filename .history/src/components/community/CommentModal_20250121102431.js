...
function CommentModal({ isOpen, onClose, post, onCommentAdded }) {
  ...
  const canDeleteComment = (comment) =>
    isCommentAuthor(comment) || user?.role === "admin";

  const handleDeleteComment = async (commentId) => {
    if (window.confirm(t("community.comments.deleteConfirm"))) {
      try {
        setLoading(true);
        // Lưu lại comments hiện tại để khôi phục nếu cần
        const currentComments = [...comments];

        // Cập nhật UI trước
        setComments((prevComments) =>
          prevComments.filter((c) => c && c.id !== commentId)
        );

        const success = await communityService.deleteComment(
          post.id,
          commentId
        );
        if (success) {
          ...
          setShowReportMenu(null); // Đóng menu sau khi xóa
        } else {
          // Khôi phục lại comments nếu xóa thất bại
