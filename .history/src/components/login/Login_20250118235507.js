  ...
  const LoginForm = () => (
    <div className="p-8">
      <div className="text-center mb-8">
        <h3 className="text-2xl font-bold text-gray-800">{t('auth.login.title')}</h3>
  ...
  const ForgotPasswordForm = () => (
    <div className="p-8">
      <div className="flex items-center mb-6">
        ...
        <span className="text-xl font-medium ml-4">{t('auth.login.forgotPasswordModal.back')}</span>
        <button
          onClick={onClose}
      ...
      <div className="text-center mb-8">
        ...
        <h3 className="text-2xl font-bold text-gray-800">{t('auth.login.forgotPasswordModal.title')}</h3>
        <p className="text-gray-600 mt-2">
          {t('auth.login.forgotPasswordModal.subtitle')}
        </p>
      </div>
      ...
      <form className="space-y-6">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            {t('auth.login.forgotPasswordModal.username')}
          </label>
          <input
            type="text"
            className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-100 focus:border-blue-400 transition-colors"
            placeholder={t('auth.login.forgotPasswordModal.usernamePlaceholder')}
          />
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            {t('auth.login.forgotPasswordModal.verificationCode')}
          </label>
          <div className="flex gap-2">
            <input
              type="text"
              className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-100 focus:border-blue-400 transition-colors"
              placeholder={t('auth.login.forgotPasswordModal.codePlaceholder')}
            />
            <button
              type="button"
              className="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors whitespace-nowrap"
            >
              {t('auth.login.forgotPasswordModal.sendCode')}
            </button>
          </div>
        </div>
        ...
        <button
          ...
          className="w-full bg-[#98E9E9] text-gray-700 py-3 rounded-lg font-medium hover:bg-[#7CD5D5] transition-colors"
        >
          {t('auth.login.forgotPasswordModal.resetPassword')}
        </button>
      </form>
    </div>
  );
