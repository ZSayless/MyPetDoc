...
function HospitalList() {
  ...
  const { hospitals: filteredHospitals } = useHospitalSearch(debouncedSearchTerm);
  const { 
    currentItems: currentHospitals,
    currentPage: paginatedCurrentPage,
    totalPages,
    setPage
  } = usePagination(filteredHospitals, hospitalsPerPage);
...
const HospitalCard = memo(({ hospital, index, baseIndex }) => {
  const { t } = useTranslation();
