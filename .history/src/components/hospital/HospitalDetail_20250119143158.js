...
const HospitalDetail = () => {
  ...
  const canPerformActions = useCallback(() => {
    // Nếu đã authenticated và có user, cho phép truy cập
    if (isAuthenticated && user) {
      // Nếu là admin, luôn cho phép
      if (user.role === 'admin') return true;
      // Nếu là user thường, cho phép
      if (user.role === 'user') return true;
      // Các role khác không cho phép
      return false;
    }
    return false;
  ...
  // Memoize handlers
  const handleLikePhoto = useCallback((photoId) => {
    if (!canPerformActions()) {
      setShowLoginModal(true);
      showToast('Please login to like photos', 'info');
      return;
    }
    ...
    showToast('Photo liked successfully');
  }, [canPerformActions, userLikes, showToast]);
  ...
  const handleAddComment = useCallback((photoId, comment) => {
    if (!canPerformActions()) {
      setShowLoginModal(true);
      return;
    }
  ...
  const handleReport = useCallback((type, targetId) => {
    if (!canPerformActions()) {
      setShowLoginModal(true);
      return;
    }
