...
function AddHospital() {
  ...
  const [location, setLocation] = useState({
    lat: 10.762622,  // Default là vị trí TP.HCM
    lng: 106.660172
  });
  ...
  const extractCoordinates = (url) => {
    try {
      // Match các pattern phổ biến của Google Maps URL
      const patterns = [
        /@(-?\d+\.\d+),(-?\d+\.\d+)/,  // Pattern cho maps.google.com
        /!3d(-?\d+\.\d+)!4d(-?\d+\.\d+)/  // Pattern cho Google Maps embed
      ];
