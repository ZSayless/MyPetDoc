...
import { MapPin, Phone, X, Star, Heart, Upload, Clock } from "lucide-react";
...
const HospitalDetail = () => {
  ...
  const handleSubmitReview = useCallback(
    async (newReview) => {
      try {
        setLoading(true);
        // Đảm bảo images được chuyển đổi thành URL
        const reviewWithImages = {
          ...newReview,
          images: newReview.images.map((imgUrl) => {
            // Nếu imgUrl đã là string (URL) thì giữ nguyên, nếu không thì tạo URL mới
        ...
        showToast(t("hospitalDetail.reviewSuccess"));
        setShowWriteReview(false);
      } catch (error) {
        showToast(t("hospitalDetail.reviewError"), "error");
      } finally {
        setLoading(false);
      }
    },
  ...
  const handleImageUpload = async (formData) => {
    ...
    try {
      ...
      showToast(t("hospitalDetail.uploadSuccess"));
      setShowUploadModal(false);
    } catch (error) {
      console.error("Error uploading photo:", error);
      showToast(t("hospitalDetail.uploadError"), "error");
    }
  };
  ...
  const handleToggleFavorite = useCallback(() => {
    ...
    if (newFavoriteState) {
      ...
      showToast(t("hospitalDetail.addToFavorites"));
    } else {
      const updatedFavorites = currentFavorites.filter((favId) => favId !== id);
      localStorage.setItem("favorites", JSON.stringify(updatedFavorites));
      showToast(t("hospitalDetail.removeFromFavorites"));
    }
  }, [canPerformActions, id, isFavorite, t]);
  ...
  if (loading) return <div className="text-center py-8">{t("hospitalDetail.loading")}</div>;
  if (error)
    return (
      <div className="min-h-screen flex items-center justify-center">
        {t("hospitalDetail.error")}: {error}
      </div>
    );
  if (!hospital)
    return (
      <div className="min-h-screen flex items-center justify-center">
        {t("hospitalDetail.hospitalNotFound")}
      </div>
    );
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex-1">
              ...
              <p className="text-sm md:text-base text-gray-600 mt-1">
                {t("hospitalDetail.specialization")}: {hospital.specialization}
              </p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 md:flex md:gap-4">
              ...
              >
                ...
                <span className="text-center">
                  {isFavorite ? (
                    <>
                      {t("hospitalDetail.removeFromFavorites")}
                      <br className="md:hidden" />
                    </>
                  ) : (
                    <>
                      {t("hospitalDetail.addToFavorites")}
                      <br className="md:hidden" />
                    </>
                  )}
                </span>
              ...
              >
                ...
                <span className="text-center">
                  {t("hospitalDetail.getDirections")}
                  <br className="md:hidden" />
                </span>
              </button>
            </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-sm p-4 md:p-6">
              <h2 className="text-lg md:text-xl font-semibold mb-4">
                {t("hospitalDetail.information")}
              </h2>
              <div className="space-y-4">
                <div className="flex items-start gap-3">
                  <MapPin className="w-5 h-5 text-gray-600 mt-1 flex-shrink-0" />
                  <div>
                    <h3 className="font-medium">{t("hospitalDetail.address")}</h3>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="font-medium">{t("hospitalDetail.phone")}</h3>
                    <p className="text-sm md:text-base text-gray-600">
                      {hospital.phone}
                    </p>
                  </div>
                ...
                <div>
                  <h3 className="font-medium mb-2">{t("hospitalDetail.workingHours")}</h3>
              ...
              >
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <span className="text-white">{t("hospitalDetail.viewDetails")}</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="relative bg-white rounded-xl w-full max-w-md p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-semibold">
                {t("hospitalDetail.report")} {reportType === "photo" 
                  ? t("hospitalDetail.photo") 
                  : t("hospitalDetail.comment")}
              </h3>
              <button
                onClick={() => setShowReportModal(false)}
                className="text-gray-500 hover:text-gray-700"
            ...
            <div className="space-y-4">
              <p className="text-gray-600">
                {t("hospitalDetail.reportReason")} {reportType === "photo" 
                  ? t("hospitalDetail.photo") 
                  : t("hospitalDetail.comment")}:
              </p>
              {[
                t("hospitalDetail.reportReasons.inappropriate"),
                t("hospitalDetail.reportReasons.spam"),
                t("hospitalDetail.reportReasons.harassment"),
                t("hospitalDetail.reportReasons.falseInfo"),
                t("hospitalDetail.reportReasons.other")
              ].map((reason) => (
                <button
                  key={reason}
