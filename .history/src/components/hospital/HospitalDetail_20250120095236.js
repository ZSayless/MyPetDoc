...
const HospitalDetail = () => {
      ...
      try {
        ...
        const convertedImages = await Promise.all(
          newReview.images.map(async (img) => {
            if (typeof img === "string") return img; // Nếu đã là string thì giữ nguyên
  ...
  const handleToggleFavorite = useCallback(() => {
    ...
    if (newFavoriteState) {
      ...
      showToast(t("hospitalDetail.alerts.favoriteAdded"));
    } else {
      const updatedFavorites = currentFavorites.filter((favId) => favId !== id);
      localStorage.setItem("favorites", JSON.stringify(updatedFavorites));
      showToast(t("hospitalDetail.alerts.favoriteRemoved"));
    }
